...
double BundlerApp::RunSFM_Ceres(int num_pts, int num_cameras, int start_camera,
...
#define MIN_POINTS 20
	...
	for (int i = 0; i < num_pts; i++) {
        ...
        for (int i = 0; i < num_projections; ++i) {
            ...
            cost_function =
                new ceres::AutoDiffCostFunction<SnavelyReprojectionError, 2, 9, 3>(
                        new SnavelyReprojectionError(projections[2 * i + 0],
                                                     projections[2 * i + 1]));
        ...
        for (int i = 0; i < num_cameras; i++) {
            ...
            prior_cost_function = 
                new ceres::AutoDiffCostFunction<PriorError, 1, 9>(
                    new PriorError(7, 0.0, m_constrain_distortion_weight * num_vis[i]));
            ...
            prior_cost_function = 
                new ceres::AutoDiffCostFunction<PriorError, 1, 9>(
                    new PriorError(8, 0.0, m_constrain_distortion_weight * num_vis[i]));
